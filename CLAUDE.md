...
uv run -m character_eng.qa_personas [--model cerebras-llama] [--character greg] [--turns 10]
...
- **QA personas**: `logs/qa_personas_{model}_{timestamp}.html` — self-contained HTML report with per-persona conversation cards, eval details, stale discard markers
