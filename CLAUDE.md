...
- **`character_eng/models.py`** — Model config registry. `MODELS` dict maps keys (`cerebras-llama`, `cerebras-gpt`, `groq-llama`, `groq-gpt`, `gemini-3-flash`, `gemini-2.5-flash`, `lfm-2.6b`, `lfm-1.2b`) to config dicts with `name`, `model`, `base_url`, `api_key_env`, `stream_usage`. Local models also have `local: True` and `path`. Models with `hidden: True` are excluded from the startup menu and benchmark auto-discovery but remain usable via `--model` flag. `DEFAULT_MODEL` is `cerebras-llama`. `PLAN_MODEL` is `gemini-3-flash` (used by the planner and reconciler).
- **`character_eng/__main__.py`** — TUI entry point. Model selection at startup (`pick_model()`), then character selection. Two nested loops: outer loop loads prompts, world state, and creates a chat session (breaks on `/reload`), inner loop handles user input and dispatches commands (`/reload`, `/trace`, `/back`, `/quit`, `/world`, `/beat`, `/plan`, `/goals`). First user input always gets a natural LLM response (no beat delivery), then triggers a replan for visitor-aware script. Subsequent turns use code-driven beat delivery: when a beat exists, `deliver_beat()` pastes the pre-rendered line directly as the character's response (no LLM call), adds it to history via `add_assistant()`. Beats with conditions are gated by `condition_check_call()` — if the condition isn't met, an idle line is displayed instead. If no beat (script empty/planner unavailable), falls back to LLM streaming. World changes (`/world <text>`) use two-speed updates: immediate pending + narrator injection + character reaction via LLM streaming, then background reconciliation. `/beat` delivers the next scripted beat if its condition is met, otherwise shows an in-character idle line. After each turn, eval decides advance/hold/off_book with optional replan. Background reconcile results are checked at the start of each turn. Uses `rich` for colored output and streaming display. Manages integrated vLLM lifecycle. Threading: background reconcile thread writes results under `_reconcile_lock`, main thread reads/clears at turn start.
...
- `tests/test_world.py` — WorldState (dict-based dynamic with stable IDs, render/apply/show/load, add_fact, add_pending/clear_pending, render_for_reconcile, render_with_pending), Beat construction (with/without gaze/expression, with condition), ConditionResult (met/not met), Script (current_beat/advance/replace/is_empty/render/render_with_gaze_expression/render_with_condition/show), EvalResult (construction/bootstrap fields), Goals (render/load from character.txt), format_narrator, format_pending_narrator, reconcile_call (6 scenarios incl. basic, no removals, empty state, multiple pending, reads reconcile_system.txt, IDs in context), eval_call (7 scenarios incl. script/goals in context, reads eval_system.txt), condition_check_call (4 scenarios incl. met/not met, condition in context, reads condition_system.txt), plan_call (6 scenarios incl. gaze/expression/condition parsing, plan_request in context, reads plan_system.txt)
