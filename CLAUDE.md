...
- **`character_eng/kitten_tts.py`** — HTTP client for Kitten-TTS-Server (ONNX, 8 fixed voices). 4-method interface. `flush()` POSTs to `{server_url}/v1/audio/speech` with `response_format: "wav"`, receives complete WAV response, extracts PCM, delivers in ~100ms chunks. 24kHz native. Only needs `requests`. Server on port 8005.
- **`character_eng/kokoro_tts.py`** — HTTP client for Kokoro-FastAPI server (82M ONNX, 54 preset voices). 4-method interface. `flush()` POSTs to `{server_url}/v1/audio/speech`, streams WAV response via `iter_content()`, extracts PCM, delivers in chunks. 24kHz native. Fastest backend at 143x realtime. Only needs `requests`. Server on port 8880.
...
tts_backend = "local"        # "elevenlabs", "local", "chatterbox", "kani", "mio", "kitten", "kokoro", or "pocket"
