...
Interactive NPC character chat CLI with selectable LLM backend (Cerebras, Groq, Google Gemini, or local vLLM models). All use OpenAI-compatible endpoints. Two-speed character planning: fast eval (chat model, every turn) tracks progress through a script, slow planner (Gemini Flash 3) generates multi-beat scripts. Seven modules:

- **`character_eng/models.py`** — Model config registry. `MODELS` dict maps keys (`cerebras-llama`, `cerebras-gpt`, `groq-llama`, `groq-gpt`, `gemini-3-flash`, `gemini-2.5-flash`, `lfm-2.6b`, `lfm-1.2b`) to config dicts with `name`, `model`, `base_url`, `api_key_env`, `stream_usage`. Local models also have `local: True` and `path`. Models with `hidden: True` are excluded from the startup menu and benchmark auto-discovery but remain usable via `--model` flag. `DEFAULT_MODEL` is `cerebras-llama`. `PLAN_MODEL` is `gemini-3-flash` (used by the planner).
- **`character_eng/__main__.py`** — TUI entry point. Model selection at startup (`pick_model()`), then character selection. Two nested loops: outer loop loads prompts, world state, and creates a chat session (breaks on `/reload`), inner loop handles user input and dispatches commands (`/reload`, `/trace`, `/back`, `/quit`, `/world`, `/think`, `/plan`, `/goals`). Code-driven beat delivery: when a beat exists, `deliver_beat()` pastes the pre-rendered line directly as the character's response (no LLM call), adds it to history via `add_assistant()`. If no beat (script empty/planner unavailable), falls back to LLM streaming. World changes (`/world <text>`) still use LLM for dynamic reactions. After each turn, eval decides advance/hold/off_book with optional replan. Uses `rich` for colored output and streaming display. Manages integrated vLLM lifecycle.
...
- **`character_eng/world.py`** — World state and script system. `WorldState` dataclass tracks static facts, mutable dynamic facts, and an event log. `Goals` dataclass holds a permanent `long_term` goal (parsed from `character.txt`). `Beat` holds a dialogue line, intent, and optional `gaze`/`expression` fields for animation/TTS pre-rendering. `Script` is an ordered beat list with `current_beat`, `advance()`, `replace()`, `is_empty()`, `render()`, `show()`. `EvalResult` holds thought, gaze, expression, and `script_status` (advance/hold/off_book/bootstrap) with optional bootstrap fields. `PlanResult` holds a list of beats. `director_call` sends a one-shot LLM call with JSON output to produce a `WorldUpdate`. `eval_call` replaces the old think system — reads `eval_system.txt`, takes script context, returns script tracking status. `plan_call` reads `plan_system.txt`, uses the plan model (Gemini Flash 3), returns a list of beats with gaze/expression. `format_narrator_message` turns updates into `[bracketed stage directions]`.
- **`character_eng/benchmark.py`** — Model speed benchmark. Runnable via `uv run -m character_eng.benchmark [--model KEY] [--runs N]`. Benchmarks three scenarios matching real usage: streaming chat (TTFT + total time), structured JSON director call, and structured JSON eval call. Uses Greg's full prompts and world state for realistic payloads. Prints per-run details and a rich summary table, saves JSON logs to `logs/`.
...
System prompts for the director, eval, and plan systems are also file-based and live-editable:
...
- `prompts/plan_system.txt` — system prompt for the conversation planner LLM (beat generation)

These are read from disk on every call, so edits take effect immediately (no `/reload` needed).

## Models

Model is chosen once at startup. Chat, director, and eval use the same model. The planner uses `PLAN_MODEL` (`gemini-3-flash`) separately — requires `GEMINI_API_KEY`. If the plan model's API key is not set, the planner is unavailable and the script system runs in eval-only mode. Available models:
...
- **Gemini 3.0 Flash** (`gemini-3-flash`) — `gemini-3-flash-preview` via Google. Does not support `stream_options`. Requires `GEMINI_API_KEY`. Also used as the plan model.
...
- `/think` — Character eval + optional beat delivery (bootstrap/advance/off_book may trigger planner)
...
- **Chat sessions**: `logs/chat_{character}_{model}_{timestamp}.json` — every send, world change, and eval with full untruncated responses
...
- `tests/test_world.py` — WorldState (render/apply/show/load), Beat construction (with/without gaze/expression), Script (current_beat/advance/replace/is_empty/render/render_with_gaze_expression/show), EvalResult (construction/bootstrap fields), Goals (render/load from character.txt), format_narrator, director_call (3 scenarios), eval_call (7 scenarios incl. script/goals in context, reads eval_system.txt), plan_call (5 scenarios incl. gaze/expression parsing, plan_request in context, reads plan_system.txt)
...
**`qa_chat`** (`uv run -m character_eng.qa_chat [--model cerebras-llama|groq-llama|groq-gpt]`) — End-to-end integration test hitting real LLM. Parses `test_plan.md` for test scenarios covering chat, world changes, eval, and prompt injection. Run manually before releases or after changes to chat/world/eval pipeline.
