...
from .streamers import AudioIteratorStreamer
...
class Qwen3TTSModel:
    ...
    def generate_voice_clone_stream(
        ...
        **kwargs,
        ...
        """
        Streamed version of generate_voice_clone. 
        Only supports single-sample generation (batch size 1).

        Returns:
            AudioIteratorStreamer: An iterator that yields (wav_chunk, sample_rate)
        """
        if self.model.tts_model_type != "base":
            raise ValueError("Streaming only supported for 'base' model type in this example.")
        ...
        # Initialize streamer
        streamer = AudioIteratorStreamer(self.model.speech_tokenizer, buffer_size=buffer_size)
