"""Kokoro — HTTP client for Kokoro-FastAPI server (ONNX, 54 preset voices).

Same 4-method interface: send_text(), flush(), wait_for_done(), close().
send_text() buffers, flush() triggers HTTP POST to /v1/audio/speech.
Uses streaming response (chunked transfer) for lower TTFA.
...
class KokoroTTS:
    ...
    Connects to a running Kokoro-FastAPI server. Uses streaming HTTP
    response to receive WAV audio incrementally.
    """
    ...
    def _generate(self, text: str):
        """Background thread: POST to Kokoro server, stream WAV response, deliver audio."""
        import requests
        ...
        try:
            ...
            # Collect the streamed response into a WAV buffer
            wav_bytes = b""
            ...
            sys.stderr.write(f"[KokoroTTS] Generation error: {e}\n")
            sys.stderr.flush()
        finally:
            self._generation_done.set()
