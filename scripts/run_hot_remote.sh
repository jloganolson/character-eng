...
LOCAL_CAMERA_HEIGHT="${LOCAL_CAMERA_HEIGHT:-360}"
LOCAL_CAMERA_JPEG_QUALITY="${LOCAL_CAMERA_JPEG_QUALITY:-60}"
...
start_camera_uplink() {
  ...
  (
    exec uv run --project services/vision python scripts/camera_uplink.py \
      ...
      --jpeg-quality "$LOCAL_CAMERA_JPEG_QUALITY" \
      --metrics-path "$TRANSPORT_VIDEO_METRICS_PATH"
  ) >"$CAMERA_LOGFILE" 2>&1 &
  echo "$!" >"$CAMERA_PIDFILE"
  sleep 2
  if ! camera_alive; then
...
env \
  ...
  CHARACTER_ENG_TRANSPORT_MODE=remote_hot \
